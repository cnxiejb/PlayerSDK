...
#ifdef XIE_LOW_DELAY
        if(!ctx->ffp->first_video_frame_rendered || pts > 3)//drop all frame
#endif
                       ...
                       CMTime presentationDuration)
{
                ...
                if (newFrame->pic.pts != AV_NOPTS_VALUE) {
                    ...
                    if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD &&
                        ...
                        if (is->continuous_frame_drops_early > ffp->framedrop) {
                            ...
                            // drop too late frame
                            av_log(NULL, AV_LOG_DEBUG, "VedeoToolBoxSync drop frame continuo_frame_drop_early=%d diff = %f\n",is->continuous_frame_drops_early,diff);
                            goto failed;
                        }
                    }
                }
    ...
    successed:
        //av_log(NULL, AV_LOG_DEBUG, "VedeoToolBoxSync got_picture\n");
        return;
    failed:
        if (newFrame) {
            free(newFrame);
...
static VTDecompressionSessionRef vtbsession_create(Ijk_VideoToolBox_Opaque* context)
{
    ...
    if(ret == -1)
        av_log(NULL, AV_LOG_DEBUG, "vtbformat_init error ret = %d\n",ret);
...
static int decode_video_internal(Ijk_VideoToolBox_Opaque* context, AVCodecContext *avctx, const AVPacket *avpkt, int* got_picture_ptr)
{
    ...
    VTDecodeInfoFlags decodeinfo;
    status = VTDecompressionSessionDecodeFrame(context->vt_session, sample_buff, decoder_flags, (void*)sample_info, &decodeinfo);
    if(kVTDecodeInfo_FrameDropped & decodeinfo)
        NSLog(@"kVTDecodeInfo_FrameDropped\n");
    if (status == noErr) {
...
static CMFormatDescriptionRef CreatFormatDescriptionFromExtradata( uint8_t *extradata,uint32_t extrasize)
{
    CMFormatDescriptionRef fmt_desc = NULL;
    
    uint8_t *sps = extradata + 6 + 2;
    uint32_t sps_size = AV_RB16(extradata + 6);
    uint8_t *pps = sps + sps_size + 1 + 2;
    uint32_t pps_size = AV_RB16(sps + sps_size + 1);
    
    const uint8_t* const parameterSetPointers[2] = { sps, pps };
    const size_t parameterSetSizes[2] = { sps_size, pps_size };
    
    if(extrasize != sps_size + pps_size + 5 + 6)
        return NULL;
        ...
        case AV_CODEC_ID_H264:
            ...
            if (extradata[0] == 1) {
                if (!validate_avcC_spc(extradata, extrasize, &fmt_desc->max_ref_frames, &sps_level, &sps_profile)) {
                    //goto failed;
                    av_log(NULL, AV_LOG_DEBUG, "validate_avcC_spc failed\n");
                }
                ...
                if(profile == FF_PROFILE_H264_CONSTRAINED_BASELINE){
                    ALOGE("%s - ConstrainedBaseline detected, level = %d \n",__FUNCTION__,level);
                }
                ...
                fmt_desc->fmt_desc = CreatFormatDescriptionFromExtradata(extradata, extrasize);
...
fail:
    ...
    NSLog(@"videotoolbox_sync_create failed\n");
    return NULL;
}
