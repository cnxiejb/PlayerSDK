    ...
    while (1) {
        ...
        if (new_packet < 0)
            ...
            if (q->is_buffer_indicator && !*finished)
                ffp_toggle_buffering(ffp, 1);//开始缓存
...
static Frame *frame_queue_peek_writable(FrameQueue *f)
{
    ...
    while (f->size >= f->max_size &&
        ...
        //av_log(NULL, AV_LOG_DEBUG, "frame_queue have not enough space to put a new frame\n");
...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    if (vp->bmp) {
        ...
        //av_log(NULL, AV_LOG_DEBUG, "video_image_display pts= %f frame_queue_nb_remaining = %d \n",vp->pts,frame_queue_nb_remaining(&is->pictq));
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            is->frame_timer += delay;//next frame_timer
            ...
            if (frame_queue_nb_remaining(&is->pictq) > 1) {
                ...
                if(!is->step && (ffp->framedrop > 0 || (ffp->framedrop && get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER)) && time > is->frame_timer + duration) {
                    av_log(NULL, AV_LOG_DEBUG, "xiejinbao rendering time drop video frame.queue_nb_remaining =%d\n",frame_queue_nb_remaining(&is->pictq));
                    frame_queue_next(&is->pictq);
                    goto retry;
                }
            }
...
#endif
        if( pts > is->cut_cached_duration_base){//随时可能丢帧，这个丢帧的基准一直在递加
            if(is->pkt_cached_duration > MIN_CACHED_DURATION)//尚未最优
                is->pkt_cached_duration -= REDUCE_CACHED_DURATION_SIZE;//减小队列长度
            is->cut_cached_duration_base += REDUCE_CACHED_DURATION_INTERVAL;
            av_log(NULL, AV_LOG_DEBUG, "\n pts = %f cut_cached_duration_base = %lld pkt_cached_duration = %d \n",pts,is->cut_cached_duration_base,is->pkt_cached_duration);
        }
        
        frame_queue_push(&is->pictq);
        //av_log(NULL, AV_LOG_DEBUG, " queue_picture pts = %f ,frame_queue_nb_remaining = %d\n",vp->pts,frame_queue_nb_remaining(&is->pictq));
...
static int get_video_frame(FFPlayer *ffp, AVFrame *frame)
{
            ...
            if (frame->pts != AV_NOPTS_VALUE) {
                ...
                if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD &&
                    ...
                    if (is->continuous_frame_drops_early > ffp->framedrop) {
                        ...
                        av_log(NULL, AV_LOG_DEBUG, "get_video_frame drop frame continuo_frame_drop_early=%d diff = %f\n",is->continuous_frame_drops_early,diff);
                        got_picture = 0;
                    }
                }
            }
    ...
    //av_log(NULL, AV_LOG_DEBUG, "get_video_frame got_picture=%d\n",got_picture);
    return got_picture;
}
...
static int decode_interrupt_cb(void *ctx)
{
    ...
    VideoState *is = ffp->is;
    //add by xie
#ifdef START_READ_FRAME_TIME
    if(is->start_read_frame_time > 0){
        double read_time_duration = (av_gettime_relative() / 1000.0) - is->start_read_frame_time;
        if(read_time_duration > 5000 && is->start_read_frame_time != is->last_start_read_frame_time){//卡顿
            
            is->pkt_cached_duration = FFMIN(is->pkt_cached_duration + MIN_CACHED_DURATION*3,MAX_CACHED_DURATION);//加大缓冲队列
            is->cut_cached_duration_base += 2*REDUCE_CACHED_DURATION_INTERVAL;//延时减小队列
            
            ffp_notify_msg2(ffp, FFP_MSG_ERROR,101);
            //av_log(NULL,AV_LOG_WARNING,"decode_interrupt start_read_frame_time = %lld read_time_duration = %f pkt_cached_duration = %d \n",is->start_read_frame_time,read_time_duration,is->pkt_cached_duration);
            is->last_start_read_frame_time = is->start_read_frame_time;//更新上次时间
        }
    }
#endif
...
static void drop_queue_until_pts(PacketQueue *q, int64_t drop_to_pts) {
    ...
    for (;;) {
        ...
        // 所以这里推流端设置好 GOP 的大小，如果 pkt_cached_duration > 2 * GOP，可以尽可能规避全部清空
...
static void control_video_queue_duration(FFPlayer *ffp, VideoState *is) {
    ...
    if (cached_duration > is->pkt_cached_duration) {
        // drop
        av_log(NULL, AV_LOG_INFO, "233 video cached_duration = %lld, nb_packets = %d.\n", cached_duration, nb_packets);
        drop_to_pts = is->videoq.last_pkt->pkt.pts - (duration / 2);  // 这里删掉一半，你也可以自己修改，依据设置进来的pkt_cached_duration大小
...
static void control_audio_queue_duration(FFPlayer *ffp, VideoState *is) {
    ...
    if (cached_duration > is->pkt_cached_duration) {
        ...
        drop_to_pts = is->audioq.last_pkt->pkt.pts - (duration * 9.0 / 10.0);//丢掉10%的数据
        drop_queue_until_pts(&is->audioq, drop_to_pts);
    }
    else if(is->cut_cached_duration_base == 2 * REDUCE_CACHED_DURATION_INTERVAL &&
            is->pkt_cached_duration > MIN_CACHED_DURATION && cached_duration < MIN_CACHED_DURATION){//一次性修正机会
        is->pkt_cached_duration = MIN_CACHED_DURATION;
        av_log(NULL, AV_LOG_DEBUG, "best network status reduce cached duration to 1.5s\n");
    }
...
static int read_thread(void *arg)
{
    ...
    is->realtime = 0;
#ifdef PKT_CACHED_DURATION
    AVDictionaryEntry *e = av_dict_get(ffp->player_opts, "pkt_cached_duration", NULL, 0);
    if (e) {
        int max_cached_duration = atoi(e->value);
        if (max_cached_duration) {
            is->pkt_cached_duration = DEFAULT_CACHED_DURATION;
            is->cut_cached_duration_base = REDUCE_CACHED_DURATION_INTERVAL;
        }
    } else {
        is->pkt_cached_duration = 0;
    }
#endif
    //end modify
...
//      of the seek_pos/seek_rel variables
        ...
        pkt->flags = 0;
#ifdef START_READ_FRAME_TIME
        is->start_read_frame_time = av_gettime_relative() / 1000;
#endif
        ret = av_read_frame(ic, pkt);
#ifdef START_READ_FRAME_TIME
        is->start_read_frame_time = 0; 
#endif
        if (ret < 0) {
            int pb_eof = 0;
            int pb_error = 0;
        ...
        // 每次读取一个视频关键帧pkt，去判断
#ifdef PKT_CACHED_DURATION
        if((pkt->flags & AV_PKT_FLAG_KEY) && !is->paused && is->pkt_cached_duration > 0 ) {
